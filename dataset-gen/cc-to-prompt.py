...
# dataset = load_dataset("PleIAs/English-PD", streaming=True)["train"]
# Modify the dataset loading to select only the columns we need
dataset = load_dataset("PleIAs/English-PD", streaming=True)["train"].select_columns([
    "identifier",
    "title",
    "publication_date",
    "word_count",
...
REQUEST:
Write a prompt based on the above text, that is a single-paragraph, high-level
description  that could be used as a prompt. Make the propmt in the format
format similar to: "Write a (chapter/piece/article/wiki entry/...), titled
(document name)', which includes (1-2 sentence list of topics to cover,
kept very vague). The full piece should be approximately
(n-paragraphs or other unit of length)".
...
output_file = "cc_prompts_3b.jsonl"
num_existing = sum(1 for _ in open(output_file)) if os.path.exists(output_file) else 0
batch = []
for data in tqdm(itertools.islice(dataset, num_existing, 100_000), total=100_000-num_existing):
    batch.append(data)
    if len(batch) >= 50:

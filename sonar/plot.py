...
def compute_cosine_similarity(ref_embeddings, comp_embeddings):
    """
    Compute the cosine similarity for each corresponding row in two arrays.

    Parameters:
        ref_embeddings (np.ndarray): Array of embeddings for the reference texts.
        comp_embeddings (np.ndarray): Array of embeddings for the comparison
        texts.

    Returns:
        np.ndarray of cosine similarity values.
    """
...
def plot_cosine_similarity_violin(df_plot, output_image):
    ...
    sns.violinplot(data=df_plot, x="Comparison Type", y="Cosine Similarity", palette="Set2", scale="width")
